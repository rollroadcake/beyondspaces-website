...
class Wizard extends Model
{
	...
	public function testSiteParams(&$siteParams)
	{
		...
		$mustSwitchDriver = strtolower($siteParams['akeeba.platform.dbdriver']) === 'mysql';

		if ($mustSwitchDriver)
		{
			$siteParams['akeeba.platform.dbdriver'] = 'pdomysql';
		}
	...
	private function flush(): array
	{
		...
		if ($this->container->appConfig->get('no_flush', 0) == 1)
		{
			return ['status' => true];
		}

		// Set no_flush to enabled
		$this->container->appConfig->set('no_flush', 1);
		...
		$this->container->appConfig->set('no_flush', 0);
		$this->container->appConfig->saveConfiguration();
