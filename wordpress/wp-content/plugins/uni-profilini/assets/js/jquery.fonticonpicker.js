...
;(function ($) {
	...
	Plugin.prototype = {
		...
		renderIconContainer: function () {
            ...
            /* mr.psiho modification */
			// If no empty icon is allowed and no current value is set or current value is not inside the icon set
			if (!this.settings.emptyIcon && (!this.element.data('chosen') || $.inArray(this.element.data('chosen'), this.settings.source) === -1)) {

				// Get the first icon
				this.setSelectedIcon(iconsPaged[0]);

			} else if ($.inArray(this.element.data('chosen'), this.settings.source) === -1) {
				...
				this.setSelectedIcon(this.element.data('chosen'));
			}
            /* end of mr.psiho modification */
