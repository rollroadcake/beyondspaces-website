		...
		$pointers = array(
			'pointers' => array(
				...
				'cpo_tab'   => array(
					...
					'next'    => 'builder_or_import',
                    /*'next_trigger' => array(
                        'target' => '.uni_cpo_settings_tab a',
                        'event'  => 'click'
                    ),*/
					'options' => array(
                ...
                'builder_or_import'   => array(
                    'target'  => '.uni_cpo_product_onboarding',
                    'open_trigger' => array(
                        'target' => '.uni_cpo_settings_tab a',
                        'event'  => 'click'
                    ),
                    'next'    => false,
                    'options' => array(
                        'content'  => '<h3>' . esc_html__( 'Go to the builder mode', 'uni-cpo' ) . '</h3>' .
                            '<p>' . esc_html__( 'Go to builder mode to start configuring your product OR import one of ready-made configuration', 'uni-cpo' ) . '</p>',
                        'position' => array(
                            'edge'  => 'left',
                            'align' => 'middle',
                        ),
			...
			"jQuery( function( $ ) {
				...
				function show_uni_cpo_pointer( id ) {
					...
					if ( pointer.next_trigger ) {
						$( pointer.next_trigger.target ).on( pointer.next_trigger.event, function(e) {
							setTimeout( function() { this_pointer.pointer( 'close' ); }, 400 );
						});
					}
					
					if (pointer.open_trigger) {
					    $( pointer.open_trigger.target ).trigger( pointer.open_trigger.event );
					}
				}
			});"
		);
