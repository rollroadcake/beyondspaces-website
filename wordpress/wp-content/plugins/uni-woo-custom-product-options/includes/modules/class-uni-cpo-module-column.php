...
class Uni_Cpo_Module_Column extends Uni_Cpo_Module implements Uni_Cpo_Module_Interface {
	...
	public static function js_template() {
		...
		<script id="js-builderius-col-tmpl" type="text/template">
            ...
            {{ const { display = 'block', flex_direction, justify_content, flex_wrap, align_items, gap = {}, width_type, float, width, content_width } = settings.general.main; }}
			...
			<div id="{{= id_name }}" class="uni-col uni-node-{{= id }} {{= class_name }}" data-node="{{= id }}" data-type="{{= type }}">
                <style>
                    .uni-node-{{= id }} {
                        {{ if ( float !== 'none' && display !== 'flex' && display !== 'inline-flex' ) { }} float: {{= float }}; {{ } else { }} clear: both; {{ } }}
	...
	public static function template( $data, $post_data = array() ) {
        ...
        <div
            ...
            <div class="uni-col-content uni-node-content uni-col-content-<?php echo !empty($main['display']) ? esc_attr($main['display']) : 'block'; ?>">
				<?php
	...
	public static function get_css( $data ) {
		...
		<?php if ( $main['float'] !== '' && (empty($main['display']) || !empty($main['display']) && 'block' === $main['display']) ) { ?> float: <?php echo esc_attr( $main['float'] ); ?>;<?php } ?>
