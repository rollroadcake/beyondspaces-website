...
/*
* Uni_Cpo_Setting_Cpo_Sc_Default class
*
*/
class Uni_Cpo_Setting_Cpo_Sc_Default extends Uni_Cpo_Setting implements Uni_Cpo_Setting_Interface {
    /**
     * Init
     *
     */
    public function __construct() {
        $this->setting_key = 'cpo_sc_default';
        $this->setting_data = array(
            'title'      => __( 'Show or hide by default?', 'uni-cpo' ),
            'is_tooltip' => true,
            'desc_tip'   => __( 'Choose whether these suboptions should be shown or hidden at the beginning,
                so its state will be changed to the opposite once the condition is met.', 'uni-cpo' ),
            'options'    => array(
                'hide' => __( 'Hide', 'uni-cpo' ),
                'show' => __( 'Show', 'uni-cpo' ),
            ),
            'js_var'     => 'data',
        );
